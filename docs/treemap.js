...
function weightedAvg(arrNodes) {

  var result = arrNodes.map(function (node, i) {

    var weight = node.value?node.value:0;
        node.percent=node.children?weightedAvg(node.children):(node.percent?node.percent:0);
    var sum = node.percent * weight;
...
function accumulate_percent(d) {
    return d.children
	? d.percent = weightedAvg(d.children)
	: (d.percent?d.percent:0);
//    return d.children
//	? d.percent = d.value?((d.children.reduce(function(accumulator, currentValue, index, array) { return (accumulator*array[index] + accumulate_percent(v)*v.value;)/ }, 0))/d.children.reduce(function(p, v) { return p + v.value; }, 0)):0
//	: d.percent;
}
