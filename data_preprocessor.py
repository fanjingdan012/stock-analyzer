...
import time
def adapt_bs(name):
    adapted_bs = pd.read_excel('../data/bs_'+name+'.xlsx')
    report_dates=[]
    df_items = []
    for i,v in adapted_bs['report_date'].items():
        timeTemp = float(v / 1000)
        tupTime = time.localtime(timeTemp)
        report_date_str = time.strftime('%Y%m%d',tupTime)
        report_dates.append(report_date_str)
    adapted_bs['report_date_str'] = report_dates
    adapted_bs2 = adapted_bs.applymap(lambda x: x.split(',', 1)[0][1:] if (isinstance(x, str) and x.startswith('[')) else x)
    adapted_bs3 = adapted_bs2.applymap(        lambda x: 0 if (x == 'None') else x)
    adapted_bs3.to_excel('../data/bs1_' + name + '.xlsx')
    return adapted_bs2
...
def merge_industry_is_cfs_bs_match_enddate_bsdate(name):
    ...
    df_merged = pd.merge(df_is_cfs_all, df_bs_all, left_on=['stock_code','enddate'], right_on = ['stock_code','report_date_str'],copy=True, indicator='exists',suffixes=('_is_cfs','_bs'))
    # df_merged.fillna(0, inplace=True)
...
def merge_industry_is_cfs_bs_match_begindate_bsdate(name):
    df_is_cfs_all = pd.read_excel('../data/is_cfs_'+name+'.xlsx', converters={'enddate': str,'begindate_is':str})
    df_is_cfs_all['is_year_report'] = df_is_cfs_all['enddate'].map(lambda d: d.endswith('1231'))
...
def calc_profit_ability(name):
    ...
    df_is_cfs_bs_begin['roe'] = df_is_cfs_bs_begin['netprofit_is']/df_is_cfs_bs_begin['total_quity_atsopc']
    ...
    df_is_cfs_bs_begin['sr_div_a'] = df_is_cfs_bs_begin['biztotinco']/df_is_cfs_bs_begin['total_assets']
